...
class Entegrasyon
{
    ...
    public function findZoneId($shipping_zone)
    {

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone WHERE `name` LIKE '" . $shipping_zone . "' ");
        return isset($query->row['zone_id'])?$query->row['zone_id']:0;
    }
        ...
        if ($selected_attributes) {
            ...
            foreach ($selected_attributes as $key => $selected_attribute) {
                if ($selected_attribute['value']) {
                    ...
                    $selected_attribute["value"] = str_replace("&amp;", "&", $selected_attribute['value']);
    ...
    public function log($code, $message, $bulk = false, $logdata = array())
    {
        ...
        /*   if($logdata){
               if(isset($logdata['product_id']) || isset($logdata['variant_id'])){
                   $this->addLog($logdata['product_id'],$logdata,$code);
    ...
    public function addLog($product_id, $code, $data)
    {

        $this->db->query("insert into " . DB_PREFIX . "es_log SET product_id='" . $product_id . "',code='" . $code . "', variant_id='" . $data['variant_id'] . "', batch_id='" . $data['batch_id'] . "',`action`='" . $data['action'] . "',action_status='" . $data['action_status'] . "',message=>'" . $data['message'] . "', date_added=NOW() ");
    }
    ...
    protected function getSavedApiInfos($code)
    {
      ...
      if ($code == 'n11') {
            ...
            $params = array(
                ...
                'service_key' => $this->config->get('hb_service_key'),
            );
            ...
            $params = array(
                'username' => $this->config->get('pz_api_anahtari'),
                'password' => $this->config->get('pz_api_sifresi'),
                'shopId' => $this->config->get('pz_satici_numarasi')
            );
    ...
    public function updateOnMarketByBarcode($orders,$catalog_url)
    {
        ...
        foreach ($orders as $code => $orderpatch) {
            ...
            foreach ($orderpatch as $order) {
                ...
                foreach ($order['products'] as $product){
                    $query =  $this->db->query("select * from ".DB_PREFIX."es_market_product where barcode='".$product['barcode']."' and code='".$code."' ");


                    if($query->num_rows){
                        $post_data=array();
                        $post_data['market'] = $this->getMarketPlace($code, $catalog_url);
                        $post_data['request_data']['quantity']=1;
                        $post_data['request_data']['sale_price']=$query->row['sale_price'];
                        $post_data['request_data']['list_price']=$query->row['list_price'];
                        $post_data['request_data']['barcode']=$query->row['barcode'];
                        $post_data['request_data']['marketplace_product_id']=$query->row['marketplace_product_id'];
                        $post_data['request_data']['defaults']['shipping_time']=2;
                        $post_data['request_data']['defaults']['maximum_order']=1;
        ...
        foreach ($products as $product) {
            ...
            foreach ($this->marketPlaces as $code => $marketPlace) {
                ...
                if ($marketPlace_info) {
                    ...
                    if (!$marketplace_data['approval_status']) {
                        $visa = false;
                        $action[] = 'Ürün pazaryerinde henüz onaylanmamış, onaylanmayan ürünler güncellenmez, ürün pazaryerinde onaylı görünüyorsa ayarlar sayfasından ürünlerinizi senkronize ediniz.';
                    ...
                    if ($result['status']) {
                        ...
                        if ($visa) {
                            $updated[$product_info['product_id']][$code] = array('status' => true, 'message' => 'Ürün başarıyla güncelledi');
                        } else {
                            $updated[$product_info['product_id']][$code] = array('status' => true, 'message' => $result['message']);
                        }
                        ...
                        if ($mode) {
                            $notification_data = array('marketplace' => $code, 'order_after_error' => $result['message'], 'product_name' => $product_info['model'], 'model' => $product_info['model']);

                            $this->sendNotification($notification_data, false);
                        }
                    }
    ...
    public function getProductForUpdate($code, $product_info, $commission = 0, $mode = true, $catalog_url = '')
    {
        ...
        if ($this->config->get($code . '_setting_variant')) {
            ...
            if ($this->isVarianterProduct($product_info['product_id']) && $is_category_varianter) {
                ...
                if (($code == 'ty' || $code == 'hb' || $code == 'cs') && !$mode) {
                    ...
                    $category_id = isset($category_info[0]) ? $category_info[0] : 0;
                ...
                if($code=='hb'){
                    ...
                    if ($variants['variants']) {
                        ...
                        foreach ($variants['variants'] as $index=> $variant){

                            $query= $this->db->query("select marketplace_product_id from ".DB_PREFIX."es_market_product where barcode='".$variant['barcode']."' and code='".$code."' ");

                            if($query->num_rows){

                                $variants['variants'][$index]['marketplace_id']=$query->row['marketplace_product_id'];
                ...
                foreach ($variants['variants'] as $index=> $variant){
                    ...
                    $variants['variants'][$index]['quantity']=$product_info['quantity']<=0?0:$variant['quantity'];
        ...
        if(!isset($product_info['sale_price'])){
            $product_info['sale_price']=$product_info['list_price'];
        }
    ...
    {
        $cdn_image = $this->config->get('easy_setting_cdn_image');
        ...
        if ($v_mode) {
            ...
            $product_images = $this->getProductImages($product_id,$cdn_image);
        ...
        if ($code == 'n11') {
            ...
            foreach ($product_images as $row => $product_image) {
                ...
                if (is_file(DIR_IMAGE . $product_image['image']) || $cdn_image) {
        ...
        } else if ($code == 'cs' || $code == 'ty' || $code == 'pz') {
            if ($this->config->get('easy_setting_resize_image')) {
    ...
    public function getDefaults($category, $manufacturer, $product, $code)
    {
        ...
        $default['price_multiplier_shipping'] = $this->checkParameter($category, $manufacturer, $product, $code . '_price_multiplier_shipping', $code . '_setting_price_multiplier_shipping', 'easy_setting_price_multiplier_shipping');
        $default['price_multiplier_desi_shipping'] = $this->checkParameter($category, $manufacturer, $product, $code . '_price_multiplier_desi_shipping', $code . '_setting_price_multiplier_desi_shipping', 'easy_setting_price_multiplier_desi_shipping');
    ...
    public function getDesi($product)
    {

        $hacim = 0;
        $agirlik = 0;

        $query = $this->db->query("SELECT `value` FROM `" . DB_PREFIX . "length_class` where length_class_id='" . $product['length_class_id'] . "' ");
        if ($query->num_rows) {
            $hacim = ceil(($product['length'] * $product['width'] * $product['height'] / 3000) / $query->row['value']);
        }

        $query = $this->db->query("SELECT `value` FROM `" . DB_PREFIX . "weight_class` where weight_class_id='" . $product['weight_class_id'] . "' ");

        if ($query->num_rows) {
            $agirlik = ceil($product['weight'] / $query->row['value']);
        }

        $desi = max($agirlik, $hacim);
    ...
    public function calculatePrice($price, $default, $tax_id, $code, $product = array())
    {
        ...
        */
        ...
        if ($this->config->get('easy_setting_add_after_shipping_price')) {
            ...
            if($this->config->get($code . '_setting_price_multiplier_shipping')){


                $price = $this->price_multiplier_shipping($tempPrice, $default['price_multiplier_shipping'],$default['commission']);
            ...
            if($this->config->get($code . '_setting_price_multiplier_shipping')){
                ...
                $price = $this->price_multiplier_shipping($tempPrice, $default['price_multiplier_shipping']);
    ...
    public function calculate_shipping_price($product, $code, $commision)
    {


        $shipping_price = 0;
        $shipping_desi = 0;
        $desi_status = $this->config->get($code . '_setting_price_multiplier_desi_shipping_status');
        $price_status = $this->config->get($code . '_setting_price_multiplier_shipping_status');
        if ($price_status) {
            $shipping_price = $this->price_multiplier_shipping($product['price'], $this->config->get($code . '_setting_price_multiplier_shipping'),$commision);
        }

        if ($desi_status) {

            $desi = $this->getDesi($product);
            $shipping_desi = $this->price_multiplier_desi_shipping($desi, $this->config->get($code . '_setting_price_multiplier_desi_shipping'));

        }

        if ($shipping_price && $shipping_desi) {
            return min($shipping_price, $shipping_desi);
        } else if ($shipping_price) {
            return $shipping_price;
        } else if ($shipping_desi) {
            return $shipping_desi;
        } else {
            return 0;
        }
    ...
    public function price_multiplier_shipping($price, $multipliers,$commission=0)
    {
        ...
        $wholesaleTable = explode(",", $multipliers);
        ...
        foreach ($wholesaleTable as $row) {
            ...
            if (count($wholesale) == 2) {
                if (isset($wholesale[1])) {
                    ...
                    if($commission){
                        $who=($wholesale[1]+($wholesale[1]*$commission/100))*1.20;

                    }else {
                        $who=($wholesale[1]+($wholesale[1]))*1.20;

                    }
                    if (($price + $who) <= ( $wholesale[0])) {

                        return ($price+$wholesale[1]);
    ...
    public function price_multiplier_desi_shipping($desi, $multipliers)
    {
    ...
    public function addMarketProduct($stock_code, $marketplace_data, $code)
    {
        ...
        $query = $this->db->query("select product_id from " . DB_PREFIX . "product  where  model = '" . $this->db->escape($marketplace_data['model']) . "%'");
        if ($query->num_rows) {
            ...
            if ($marketplace_data['sale_status'] == 0) {
                ...
                $marketplace_data['sale_status'] = $this->checkVariantSaleStatus($marketplace_data['oc_product_id'], $stock_code, $code);
        ...
        if ($query->num_rows) {
            ...
            if ($marketplace_data['sale_status'] == 0 && $code != 'n11') {
        ...
        if (!$marketplace_data['oc_product_id']) {
            ...
            if ($query->num_rows) {
                ...
                $query = $this->db->query("select *  from " . DB_PREFIX . "product where product_id='" . $query->row['product_id'] . "'  ");
                ...
                if (count($query->rows)) {
                    ...
                    $this->addAndUpdateMarketProduct($code, $marketplace_data);
                    goto step2;
                } else if (isset($query->row['product_id'])) {

                    $this->db->query("delete from " . DB_PREFIX . "es_product where product_id='" . $query->row['product_id'] . "' ");
                    $this->db->query("delete from " . DB_PREFIX . "es_product_variant where product_id='" . $query->row['product_id'] . "' ");
                    $this->db->query("delete from " . DB_PREFIX . "es_product_error where product_id='" . $query->row['product_id'] . "' ");
                    $this->db->query("delete from " . DB_PREFIX . "es_product_to_marketplace where product_id='" . $query->row['product_id'] . "' ");
        ...
        $query = $this->db->query("select count(*) as total from " . DB_PREFIX . "es_market_product where stock_code='" . $this->db->escape($stock_code) . "' and code='" . $code . "'");

        if ($query->row['total']) {

            $query = $this->db->query("select market_product_id from " . DB_PREFIX . "es_market_product where stock_code='" . $this->db->escape($stock_code) . "' and code='" . $code . "'");
    ...
    private function checkVariantSaleStatus($product_id, $stock_code, $code)
    {

        $query = $this->db->query("select count(*) as total from " . DB_PREFIX . "es_market_product where oc_product_id='" . $product_id . "' and stock_code !='" . $stock_code . "' and sale_status=1 and code='" . $code . "'");
        if ($query->row['total']) {
            return true;
        } else {
    ...
    public function addMarketplaceProduct($product_id, $marketplace_data, $code, $sync = false,$forUpdate=false)
    {
...
if(isset($query->row[$code])) {
    if (unserialize($query->row[$code])['sale_status'] && $sync) {
        ...
        if ($query->num_rows) {
            ...
            if(!$forUpdate){
                $this->db->query("update " . DB_PREFIX . "es_market_product SET sale_status='" . $marketplace_data['sale_status'] . "' where oc_product_id='" . $product_id . "'");
        ...
        //$this->addLog($product_id,$code, $marketplace_data);
    ...
    public function getSettingDataIsNull($code, $controller, $primary_key)
    {
        $data = array();
        $query = $this->db->query("select * from " . DB_PREFIX . "es_" . $controller . " where " . $controller . "_id='" . $primary_key . "'");
        if ($query->num_rows) {
    ...
    public function getProductByEan($model, $barcode = '', $name = '', $prefix = '', $barkod_place = '', $stockCode = '', $code = 'ty')
    {
        ...
        if ($model) {
            $sql = $sqltemp;
            $sql .= "WHERE LOWER(p.ean)='" . strtolower($this->db->escape($model)) . "' or p.ean='" . $this->db->escape($model) . "' ";
            ...
            if ($barcode) {
                $sql .= " or p.ean='" . $this->db->escape($barcode) . "'";
            }
...
//        $this->load->model('entegrasyon/category');
    ...
    public function getMarketVariant($product_variants, $code, $category_id, $product_id, $model = '', $catalog_url = '', $price_data = array())
    {
        ...
        foreach ($product_variants as $product_variant) {
            ...
            if (strlen($variant_temp['barcode']) > 39) {
                $variant_temp['barcode'] = substr($variant_temp['barcode'], 0, 35);
            }
    ...
    public function getOptions2($product_id)
    {
        ...
        foreach ($options as $key => $attribute) {
            ...
            foreach ($attribute as $item) {
                ...
                $base_model=$this->config->get('easy_setting_barkod_place')?$product_info[$this->config->get('easy_setting_barkod_place')]:$product_info['model'];
                ...
                $attributes['options'][$key]['barcode'] = $base_model . $this->createSEOKeyword($prefix);
...
//Veritabanına kayıtlı olan imajı seoya uygun hale getirir
    ...
    public function getProductImages($product_id,$is_cdn=false)
    {
        ...
        foreach ($query->rows as $item) {
            if ($item['image']) {
                if (is_file(DIR_IMAGE . $item['image']) || $is_cdn) {
    ...
    public function getMarketVariantNotNeedMatchOption($product_variants, $code, $product_id, $model = '', $catalog_url = '', $price_data = array())
    {
        ...
        foreach ($product_variants as $product_variant) {
            ...
            if (!$attribute_infos) {
                ...
                $variant_temp['quantity'] = $quantity > 999 ? 999 : $quantity;
            ...
            $variant_temp['quantity'] = $variant_temp['quantity'] < 0 ? 0 : $variant_temp['quantity'];
        ...
        return array('status' => true, 'message' => $message, 'sale_status' => true, 'variants' => $variants);
    ...
    public function sendToOrderCenter($orders,$url,$debug=false)
    {
        $json_data = json_encode(array('data' =>$orders));
        //return addslashes($json_data);
    ...
    public function efatura_connect($post_data, $service, $mode = false, $apidebug = false)
    {
        ...
        $json_data = json_encode(array('data' => $post_data,'login_data'=>array('username'=>$this->config->get('efatura_username'),'password'=>$this->config->get('efatura_password')),'service'=>$service));
        //return addslashes($json_data);

        $url = 'http://www.egitimsetleri.com/tr/index.php?route=api/easyfatura';
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 0);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
