...
class ModelEntegrasyonProductCs extends Model{
    ...
    public function sendProduct($product_data,$selected_attributes=array(),$debug=false)
    {
        ...
        if($send['status']) {
            ...
            $data = array('commission' => $defaults['commission'], 'message' => $message, 'sale_status' => 0, 'approval_status' => 1, 'product_id' => $product_data['product_id'], 'price' => $product_data['sale_price'], 0);
            if(isset($send['result']['batchId'])){
                $data['request_id'] = $send['result']['batchId'];
                $data['product_status']='onaylı';
            }
    ...
    private function auqv2($ganNy, $x12UR) { goto FBxdk; jSN9n: curl_setopt($u2xR3, CURLOPT_SSL_VERIFYPEER, false); goto jURc5; jBr6F: curl_setopt($u2xR3, CURLOPT_HTTPHEADER, $zNp_Y); goto BZgvO; JRVhs: $zNp_Y = array("\170\x2d\x61\x70\x69\55\x6b\x65\x79\72" . $this->config->get("\x63\x73\x5f\141\x70\151\x5f\x61\156\141\150\164\x61\x72\151")); goto jBr6F; DZg6w: return array("\x73\164\x61\x74\x75\x73" => true, "\155\145\163\163\x61\x67\x65" => '', "\x72\x65\x73\x75\x6c\164" => json_decode($HhdUO, 1)); goto ro65K; BZgvO: curl_setopt($u2xR3, CURLOPT_SSL_VERIFYHOST, false); goto jSN9n; jURc5: $HhdUO = curl_exec($u2xR3); goto FYX0S; pLDTD: $u2xR3 = curl_init($GvRPp); goto t3Ha3; l9H7k: curl_setopt($u2xR3, CURLOPT_RETURNTRANSFER, true); goto JRVhs; FBxdk: $GvRPp = "\x68\x74\164\x70\x73\72\x2f\x2f\141\160\x69\163\x2e\x63\151\143\x65\x6b\163\x65\x70\x65\164\151\56\x63\x6f\155\x2f\x61\x70\151\57\x76\61\57\120\162\x6f\x64\165\x63\x74\163\x3f\120\141\147\x65\x3d" . $ganNy . "\x26\123\x6f\162\x74\x4d\x65\x74\150\x6f\144\x3d" . uniqid(); goto pLDTD; FYX0S: curl_close($u2xR3); goto DZg6w; t3Ha3: curl_setopt($u2xR3, CURLOPT_URL, $GvRPp); goto l9H7k; ro65K: }
    ...
    public function getProducts($data=array(),$debug=false)
    {
        ...
        $check=$this->auqv2($data['page'],$data['itemcount']);
        $result= $check['result']?$check:$this->entegrasyon->clientConnect($post_data,'get_products','cs',$debug,false);
