...
class ControllerEntegrasyonCategory extends Controller
{
    ...
    public function setting()
    {
        ...
        if (isset($data[$code . '_category_id'])) {
            ...
            if ($val[0]) {
                ...
                if ($code == 'pz' || $code == 'ty' || $code == 'hb' || $code == 'cs') {
    ...
    public function match_value_form()
    {
        ...
        //kullanıcının manuel eşleştirme verileri
        // $this->load->model('setting/setting');
        //$this->model_setting_setting->deleteSetting($code.'_reference');
        $reference_matched_map=(array)$this->config->get($code.'_reference_matched_map');
...
//seçenek değeri yok o yüzden tüm seçenekleri eşleşmiş sayıyoruz
                    foreach ($data['oc_option_values'] as $oc_option_value) {
                        ...
                        if (!$query->num_rows) {
                            ...
                            $this->db->query("INSERT INTO " . DB_PREFIX . "es_option_value SET matched_option_id='" . $matched_option_id . "', oc_option_value_id='" . $oc_option_value['option_value_id'] . "', market_option_value_id='" . $market_option_value_info['market_option_value_id'] . "',market_option_value_name='" . $market_option_value_info['market_option_value_name'] . "',market_option_value_order=1");
                    ...
                    //seçenek değeri yok o yüzden tüm seçenekleri eşleşmiş sayıyoruz- sonu
        ...
        foreach ($data['oc_option_values'] as $oc_option_value) {
            ...
            if (in_array(mb_strtoupper(mb_strtoupper($oc_option_value['name'])), $market_option_values)) {
                $market_option_value_info=array();
                ...
                if(array_key_exists($oc_option_value['option_value_id'],$reference_matched_map) ){

                    // echo 'akıllı referans sisteminden kayıt tespit edildi';
                    $market_option_value_info =  $reference_matched_map[$oc_option_value['option_value_id']];
                    if(in_array($market_option_value_info['market_option_value_name'],$market_option_values)){
                    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "es_option_value where matched_option_id='" . $matched_option_id . "' and oc_option_value_id='" . $oc_option_value['option_value_id'] . "'");
                    if (!$query->num_rows) {
                        // print_r($market_option_value_info);
                        $this->db->query("INSERT INTO " . DB_PREFIX . "es_option_value SET matched_option_id='" . $matched_option_id . "', oc_option_value_id='" . $oc_option_value['option_value_id'] . "', market_option_value_id='" . $market_option_value_info['market_option_value_id'] . "',market_option_value_name='" . $market_option_value_info['market_option_value_name'] . "',market_option_value_order=3");
                    }
                    }
...
//return;
    ...
    public function attributes()
    {
        ...
        if ($category_id) {
            ...
            if ($this->config->get($code . '_setting_variant') && $is_product_varianter && ($code == 'ty' || $code == 'pz'  || $code == 'cs')) {
                $attributes = $this->model_entegrasyon_category->getAttributes($category_id, $code, true);
    ...
    public function save_option_value()
    {
        ...
        //Kaydedilmiş Referans verileri
        $reference_matched_map=$this->config->get($code.'_reference_matched_map');
        ...
        if ($market_option_value_info) {
            ...
            //kullanıcı tarafıdan yapılan seçenek eşleştirme verilerini, otomatik eşleştirmelere referans olması için kaydediyoruz.
            $this->load->model('setting/setting');
            $reference_matched_map[$oc_option_value_id]=$data;
            $this->model_setting_setting->editSetting($code.'_reference',array($code.'_reference_matched_map'=>$reference_matched_map));
