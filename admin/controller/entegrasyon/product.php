...
class ControllerEntegrasyonProduct extends Controller
{
    ...
    protected function getList()
    {
        ...
        $product_total =$this->model_entegrasyon_product->getTotalProducts($filter_data); //count($this->model_entegrasyon_product->getProducts($filter_data, "for_bulk"));
        ...
        foreach ($results as $result) {
            ...
            $product_setting_is_null = array(
                //
                'n11' => $this->model_entegrasyon_product->getSettingDataIsNull("n11",'product', $result['product_id']),
                'hb' => $this->model_entegrasyon_product->getSettingDataIsNull("hb",'product', $result['product_id']),
                'ty' => $this->model_entegrasyon_product->getSettingDataIsNull("ty",'product', $result['product_id']),
                'eptt' =>$this->model_entegrasyon_product->getSettingDataIsNull("eptt",'product', $result['product_id']),
                'amz' => $this->model_entegrasyon_product->getSettingDataIsNull("amz",'product', $result['product_id']),
                'cs' => $this->model_entegrasyon_product->getSettingDataIsNull("cs",'product', $result['product_id']),
                'pz' => $this->model_entegrasyon_product->getSettingDataIsNull("pz",'product', $result['product_id']),
            );



            $data['products'][] = array(
                'product_setting_is_null' => $product_setting_is_null,
                'product_id' => $result['product_id'],
                'image' => $image,
                'n11' => unserialize($result['n11']),
                'hb' => unserialize($result['hb']),
                'ty' => unserialize($result['ty']),
                'pz' => unserialize($result['pz']),
        ...
        $url .= '&filter_price_prefix=' . html_entity_decode($this->request->get['filter_price_prefix']);
    }
    ...
    public function get_update_system_url()
    {
        $code=$this->request->get['code'];
        $url_prefix='update';
        if($this->config->get($code.'_setting_update_system')){
            $url_prefix='product';
        }

        echo json_encode(array('url'=>$url_prefix));
    }
    ...
    public function addproduct($product_id = null, $code = null, $bulk = false)
    {
        ...
        // $desi = $this->entegrasyon->getDesi($product_info);



        /* if ($this->config->get('easy_setting_only_list_price_multiplier')){
             $product_data['list_price'] = $product_info['price'];
         }else{ */
    ...
    public function send_to_marketplace()
    {
        ...
        if ($this->config->get('easy_setting_only_list_price_multiplier') && $product_info['special'] && $defaults['product_special']){
            $product_data['list_price'] = $product_info['price'];
        }else{
            $product_data['list_price'] = $this->entegrasyon->calculatePrice($product_info['price'], $product_data['defaults'], $product_info['tax_class_id'], $code, $product_info);
        ...
        if($code=='cs'){
            $product_data['model']=str_replace(array('/','+',' '),'',$product_data['model']);
    ...
    public function update_products_after_modified()
    {
        ...
        foreach ($results as $products) {
            foreach ($products as $code => $result) {
                ...
                $json[] = array('status' => $result['status'], 'marketplace' => $this->entegrasyon->marketPlaces[$code], 'message' => isset($result['message']) ?  $result['message']:'Başarıyla Güncellendi');
            }
    ...
    public function update($product_id = null, $code = null, $bulk = false, $price = false)
    {
        ...
        $post_data['market'] = $this->model_entegrasyon_general->getMarketPlace($code);
        $is_varianter = $this->entegrasyon->isVarianterProduct($product_info['product_id']);
        ...
        if ($marketplace_data['approval_status'] && !$marketplace_data['sale_status'] && $product_info['quantity'] > 0) {
        ...
        if ($visa) {
            ...
            $json['status']=false;
            $json['message'] = $product_info['name'] . implode(',',$action);
            if ($bulk) {
                return $json;
        ...
        if ($result['status']) {
            ...
            } else if ($is_varianter) {

                if ($product_info['variants']['sale_status']) {
                    $marketplace_data['sale_status'] = 0;
                }
...
//print_r( $data['s_attr']);
    ...
    public function send_bulk()
    {
        ...
        $in_marketplaces_product_total = $this->model_entegrasyon_product->getTotalProducts($filter_data,"",true);
        $product_filter_oc_total = $this->model_entegrasyon_product->getTotalProducts($filter_data,"",2);

        $data['product_total'] = $product_total;
        $data['in_marketplaces_product_total'] = $in_marketplaces_product_total;
        $data['product_filter_oc_total'] = $product_filter_oc_total;
   ...
   */
    ...
    public function get_marketplace_products_new()
    {
        ...
        foreach ($products as $product) {
            ...
            if ($code == 'cs' || $code == "n11") {
                if (isset(unserialize($product['custom_data'])['stockItems']['stockItem']['images'])){
                    if ((unserialize($product['custom_data'])['stockItems']['stockItem']['images'])){
                        $product['image'] = unserialize($product['custom_data'])['stockItems']['stockItem']['images'][0];
                    }else{
    ...
    public function download_product()
    {
        ...
        //print_r($marketplace_product_info);
