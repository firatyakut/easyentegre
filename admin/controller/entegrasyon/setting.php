...
class ControllerEntegrasyonSetting extends Controller
{
    ...
    public function __construct($registry)
    {
        ...
        $this->order_ranges[]=array("index"=>"-1 day", "value"=>"1 gün önce");
        $this->order_ranges[]=array("index"=>"-3 day", "value"=>"3 gün önce");
        $this->order_ranges[]=array("index"=>"-7 day", "value"=>"7 gün önce");
        $this->order_ranges[]=array("index"=>"-15 day", "value"=>"15 gün önce");
        $this->order_ranges[]=array("index"=>"-30 day ", "value"=>"1 ay önce");
        $this->order_ranges[]=array("index"=>"-60 day ", "value"=>"2 ay önce");
    ...
    public function updatedbforProductVariants()
    {
        ...
        $this->db->query("delete from ".DB_PREFIX."es_product_variant");
        $this->load->model('entegrasyon/product');
        $products=$this->model_entegrasyon_product->getProducts();
        foreach ($products as $product){
            $query=$this->db->query("select count(*) as total from ".DB_PREFIX."product_option where product_id='".$product['product_id']."'");
            if($query->row['total']){
                $this->entegrasyon->getPoductVariants($product['product_id']);
            }
        }
    ...
    public function checkmode()
    {
        ...
        if (!$mode_info) {
            $mode_info = array("domain_control" => 1, "easy_visibility" => "0", "easy_ticket" => "0");
        }
    ...
    public function cart()
    {
        ...
        if ($total == 1) {
            $data['total'] = 3250;
            $data['list_price'] = 3250;
            $data['discount'] = 0;
        } else if ($total == 2) {
            $data['total'] = 6500;
            $data['list_price'] = 6500;
            $data['discount'] = 0;
        } else if ($total == 3) {
            $data['total'] = 9750;
            $data['list_price'] = 9750;
            $data['discount'] = 0;
        } else if ($total == 4) {
            $data['total'] = 13000;
            $data['list_price'] = 13000;
            $data['discount'] = 0;
        } else if ($total == 5) {
            $data['total'] = 16350;
            $data['list_price'] = 16350;
            $data['discount'] = 0;
        }
    ...
    public function marketplaces()
    {
        ...
        $data['easy_visibility'] = $this->config->get('easy_visibility') ? '' : ' hidden ';
        $marketplaces = $this->model_entegrasyon_general->getMarketPlaces();
        $data['easy_setting_sync_mode'] = (boolean)$this->config->get('easy_setting_sync_mode');


        // echo $data['easy_setting_sync_mode'];
        $data['marketplaces'] = array();
        ...
        if ($this->config->get('mir_login')) {
            ...
            foreach (  $data['marketplaces'] as $marketplace) {
                if($marketplace['status']){
                    ...
                    $data['orders_update_cron'][] = array('name'=>$marketplace['name'],'link'=>HTTPS_CATALOG . 'index.php?route=entegrasyon/update/update_orders&code='.$marketplace["code"]);
    ...
    public function sync_form()
    {
        ...
        try {
            $matched_total = $this->db->query("select count(*) as total from " . DB_PREFIX . "product p left join " . DB_PREFIX . "es_product_to_marketplace p2m ON (p.product_id=p2m.product_id) where p2m." . $code . " != '' ")->row['total'];
            $unMatcheds = $this->db->query("select p.product_id from " . DB_PREFIX . "product p left join " . DB_PREFIX . "es_product_to_marketplace p2m ON (p.product_id=p2m.product_id) where p2m." . $code . " ='' ")->rows;;
        ...
        $data['unmatched'] = count($unMatcheds);

        // $this->session->data[$code.'_unmatched']=$unMatcheds;

        $data['total'] = $data['unmatched'];
        $data['token_link'] = $this->token_data['token_link'];
        $data['code'] = $code;

        $marketPlace = $this->model_entegrasyon_general->getMarketPlace($code);
        $data['marketplace'] = $marketPlace['name'];
        $data['info'] = 'Kataloğunuzda yer alan toplam eşleşen ve eşleşmeyen ürün toplamı aşağıda listelenmiştir. Eşleşmeyen ürünlerinizi ' . $marketPlace['name'] . ' mağanızdai ürünlerinizle eşleştirmek için aşağıdaki eşleştir butonuna tıklayınız.  ';

        $this->response->setOutput($this->load->view('entegrasyon/sync/sync_form', $data));
    ...
    public function sync_progress()
    {
        ...
        if ($code == "n11") $limit=30;
        $start = $page * $limit;

        //işleme tabi tutulacak product_id'leri session'dan çekiyoruz
        // $unMatcheds=$this->session->data[$code.'_unmatched'];
        $unMatcheds = $this->db->query("select p.product_id from " . DB_PREFIX . "product p left join " . DB_PREFIX . "es_product_to_marketplace p2m ON (p.product_id=p2m.product_id) where p2m." . $code . " ='' limit $start,$limit  ")->rows;;


        foreach ($unMatcheds as $unMatched_i) {

            $product_id = $unMatched_i['product_id'];
            $product_info = $this->entegrasyon->getProduct($product_id, $code);

            //Ürün sorgulaması için ürüne ait barkod bilgisini tespit ediyoruz
            $barcode = $this->config->get($code . '_setting_model_prefix') . $product_info['model'];

            if ($this->config->get($code . '_setting_barkod_place')) {

                if ($product_info[$this->config->get($code . '_setting_barkod_place')]) {
                    $barcode = $product_info[$this->config->get($code . '_setting_barkod_place')];
                }
            ...
            /* if ($this->config->get($code . '_setting_main_product_id')) {
                 ...
                 if ($product_info[$this->config->get($code . '_setting_main_product_id')]) {
                     $barcode = $product_info[$this->config->get($code . '_setting_main_product_id')];
                 }
            ...
            if (isset($product_setting[$code . '_barcode'])) {

                $barcode = $product_setting[$code . '_barcode'];
        ...
        // print_r($barcodes);return;

        $this->load->model('entegrasyon/product/' . $code);


//Ürünü pazaryerinde sorgulatıyoruz
        $market_products = $this->{"model_entegrasyon_product_" . $code}->getProduct($barcodes, false);


        $thisUnmatched = $limit;


        if(isset($market_products[0])) {

            foreach ($market_products as $market_product) {

                if (!$market_product['stock_code']) {
                    $market_product['stock_code'] = $market_product['barcode'];
                }

                $result = $this->entegrasyon->addMarketProduct($market_product['stock_code'], $market_product, $code, true);
        ...
        $unMatched = $unMatched + $thisUnmatched;

        $page++;


        $url = 'index.php?route=entegrasyon/setting/sync_progress&page=' . $page . '&total=' . $total . '&code=' . $code . '&matched=' . $matched . '&unmatched=' . $unMatched . '&' . $this->token_data['token_link'];
        $json['matched'] = $matched;
        $json['unmatched'] = $unMatched;
        $json['page'] = $page;
        $json['total_page'] = $total / $limit;
    ...
    public function get_products_form()
    {
        ...
        $products = $this->{"model_entegrasyon_product_" . $code}->getProducts(array('itemcount' => 10, 'page' => 0));
    ...
    public function get_products_progress()
    {
        ...
        $pasif = false;
        if (isset($this->request->get['pasif']) && $this->request->get['pasif']) {
            $pasif = true;
        ...
        if ($code == 'cs') {
            ...
            $limit = $this->config->get('hb_setting_maximum_product') ? $this->config->get('hb_setting_maximum_product') : 100;
        ...
        foreach ($render_product as $product) {
            ...
            $result = $this->entegrasyon->addMarketProduct($product['stock_code'], $product, $code, true);
        ...
        $url = 'index.php?route=entegrasyon/setting/get_products_progress&pasif=' . $pasif . '&page=' . $page . '&total=' . $datam['total'] . '&code=' . $code . '&totalDownloaded=' . $totaldownloaded . '&matched=' . $matched . '&unmatched=' . $unmatched . '&downloaded=' . $downloaded . '&updated=' . $updated . '&' . $this->token_data['token_link'];
    ...
    public function edit_n11()
    {
        ...
        $data['n11_setting_main_product_id'] = $this->config->get('n11_setting_main_product_id');
        ...
        $data['n11_setting_price_multiplier_shipping'] = $this->config->get('n11_setting_price_multiplier_shipping');
        $data['n11_setting_price_multiplier_shipping_status'] = $this->config->get('n11_setting_price_multiplier_shipping_status');
        $data['n11_setting_price_multiplier_desi_shipping'] = $this->config->get('n11_setting_price_multiplier_desi_shipping');
        $data['n11_setting_price_multiplier_desi_shipping_status'] = $this->config->get('n11_setting_price_multiplier_desi_shipping_status');
        $data['n11_setting_order_status_id'] = $this->config->get('n11_setting_order_status_id');
        $data['n11_setting_order_range'] = $this->config->get('n11_setting_order_range');
        $data['order_ranges'] = $this->order_ranges;
    ...
    public function chance_shipping_method_status()
    {
        $index=$this->request->post['index'];
        $status=$this->request->post['status'];

        $query = $this->db->query("select * from ".DB_PREFIX."setting where `key` = '".$index."'  ");

        if($query->num_rows){
            //var
            $this->db->query("update ".DB_PREFIX."setting SET value = '".$status."' where `key` = '".$index."' ");
    ...
    public function edit_gg()
    {
        ...
        $data['gg_setting_price_multiplier_shipping'] = $this->config->get('gg_setting_price_multiplier_shipping');
    ...
    public function edit_cs()
    {
        ...
        $data['cs_setting_price_multiplier_shipping'] = $this->config->get('cs_setting_price_multiplier_shipping');
        $data['cs_setting_update_system'] = $this->config->get('cs_setting_update_system');
        $data['cs_setting_price_multiplier_desi_shipping'] = $this->config->get('cs_setting_price_multiplier_desi_shipping');
        $data['cs_setting_price_multiplier_desi_shipping_status'] = $this->config->get('cs_setting_price_multiplier_desi_shipping_status');
        $data['cs_setting_price_multiplier_shipping_status'] = $this->config->get('cs_setting_price_multiplier_shipping_status');
        $data['cs_setting_order_status_id'] = $this->config->get('cs_setting_order_status_id');
        $this->load->model('entegrasyon/order');
        $data['order_statuses']=$this->model_entegrasyon_order->getOrderStatuses();
        $data['cs_setting_order_range'] = $this->config->get('cs_setting_order_range');
...
//  print_r($data);
    ...
    public function edit_ty()
    {
        ...
        $data['ty_setting_update_system'] = $this->config->get('ty_setting_update_system');
        $data['ty_setting_price_multiplier_shipping'] = $this->config->get('ty_setting_price_multiplier_shipping');
        $data['ty_setting_price_multiplier_desi_shipping'] = $this->config->get('ty_setting_price_multiplier_desi_shipping');
        $data['ty_setting_price_multiplier_desi_shipping_status'] = $this->config->get('ty_setting_price_multiplier_desi_shipping_status');
        $data['ty_setting_price_multiplier_shipping_status'] = $this->config->get('ty_setting_price_multiplier_shipping_status');
        $data['ty_setting_order_status_id'] = $this->config->get('ty_setting_order_status_id');
        $data['ty_setting_order_range'] = $this->config->get('ty_setting_order_range');
        $this->load->model('entegrasyon/order');
        $data['order_statuses']=$this->model_entegrasyon_order->getOrderStatuses();
        $data['order_ranges'] = $this->order_ranges;
    ...
    public function edit_pz()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $this->load->model('setting/setting');
            $this->model_setting_setting->editSetting('pz_setting', $this->request->post);
            echo json_encode(array('status' => true));
            return;
        }

        $data['token_link'] = $this->token_data['token_link'];
        $data['pz_setting_commission'] = str_replace("%", "", $this->config->get('pz_setting_commission'));
        $data['pz_setting_color'] = $this->config->get('pz_setting_color');
        $data['pz_setting_product_attribute'] = $this->config->get('pz_setting_product_attribute');
        $data['pz_setting_kdv_setting'] = $this->config->get('pz_setting_kdv_setting');
        $data['pz_setting_additional_content'] = $this->config->get('pz_setting_additional_content');
        $data['pz_setting_product_special'] = $this->config->get('pz_setting_product_special');
        $data['pz_setting_shipping_price'] = $this->config->get('pz_setting_shipping_price');
        $data['pz_setting_model_prefix'] = $this->config->get('pz_setting_model_prefix');
        $data['pz_setting_variant'] = $this->config->get('pz_setting_variant');
        $data['pz_setting_barkod_place'] = $this->config->get('pz_setting_barkod_place');
        $data['pz_setting_main_product_id'] = $this->config->get('pz_setting_main_product_id');
        $data['pz_setting_product_category'] = $this->config->get('pz_setting_product_category');
        $data['pz_setting_product_iscurrency'] = $this->config->get('pz_setting_product_iscurrency');
        $data['pz_setting_oc_order'] = $this->config->get('pz_setting_oc_order');
        $data['pz_setting_customer_group'] = $this->config->get('pz_setting_customer_group');
        $data['pz_setting_add_tc'] = $this->config->get('pz_setting_add_tc');
        $data['pz_setting_shipping_company'] = $this->config->get('pz_setting_shipping_company');
        $data['pz_setting_returning_address'] = $this->config->get('pz_setting_returning_address');
        $data['pz_setting_shipping_address'] = $this->config->get('pz_setting_shipping_address');
        $data['pz_setting_price_multiplier'] = $this->config->get('pz_setting_price_multiplier');
        $data['pz_setting_update_system'] = $this->config->get('pz_setting_update_system');
        $data['pz_setting_price_multiplier_shipping'] = $this->config->get('pz_setting_price_multiplier_shipping');
        $data['pz_setting_price_multiplier_desi_shipping'] = $this->config->get('pz_setting_price_multiplier_desi_shipping');
        $data['pz_setting_price_multiplier_desi_shipping_status'] = $this->config->get('pz_setting_price_multiplier_desi_shipping_status');
        $data['pz_setting_price_multiplier_shipping_status'] = $this->config->get('pz_setting_price_multiplier_shipping_status');
        $data['pz_setting_order_status_id'] = $this->config->get('pz_setting_order_status_id');
        $data['pz_setting_order_range'] = $this->config->get('pz_setting_order_range');
        $this->load->model('entegrasyon/order');
        $data['order_statuses']=$this->model_entegrasyon_order->getOrderStatuses();
        $data['order_ranges'] = $this->order_ranges;
        ...
        $this->response->setOutput($this->load->view('entegrasyon/setting/mainsetting/pz', $data));
...
//  print_r($data);
    ...
    public function edit_hb()
    {
        ...
        $data['hb_setting_maximum_product'] = $this->config->get('hb_setting_maximum_product');
        $data['barkod_places'] = array('sku', 'mpn', 'upc', 'jan', 'ean', 'isbn');
        $data['hb_setting_price_multiplier'] = $this->config->get('hb_setting_price_multiplier');
        $data['hb_setting_price_multiplier_shipping'] = $this->config->get('hb_setting_price_multiplier_shipping');
        $data['hb_setting_update_system'] = $this->config->get('hb_setting_update_system');
        $data['hb_setting_price_multiplier_desi_shipping'] = $this->config->get('hb_setting_price_multiplier_desi_shipping');
        $data['hb_setting_price_multiplier_desi_shipping_status'] = $this->config->get('hb_setting_price_multiplier_desi_shipping_status');
        $data['hb_setting_price_multiplier_shipping_status'] = $this->config->get('hb_setting_price_multiplier_shipping_status');
        $data['hb_setting_order_range'] = $this->config->get('hb_setting_order_range');


        $data['hb_setting_order_status_id'] = $this->config->get('hb_setting_order_status_id');
        $this->load->model('entegrasyon/order');
        $data['order_statuses']=$this->model_entegrasyon_order->getOrderStatuses();
    ...
    public function edit_easy()
    {
        ...
        $data['easy_setting_only_list_price_multiplier'] = $this->config->get('easy_setting_only_list_price_multiplier');
        ...
        $data['easy_setting_main_product_id'] = $this->config->get('easy_setting_main_product_id');
        $data['easy_setting_model_prefix'] = $this->config->get('easy_setting_model_prefix');
        ...
        $data['easy_setting_order_center_url'] = $this->config->get('easy_setting_order_center_url');
        $data['easy_setting_order_center_customer_id'] = $this->config->get('easy_setting_order_center_customer_id');
        ...
        $data['easy_setting_sync_mode'] = $this->config->get('easy_setting_sync_mode');
        $data['easy_setting_cdn_image'] = $this->config->get('easy_setting_cdn_image');
        $data['easy_setting_stock_always_1'] = $this->config->get('easy_setting_stock_always_1');
        $data['barkod_places'] = array('sku', 'mpn', 'upc', 'jan', 'ean', 'isbn');
    ...
    public function edit_eptt()
    {
        ...
        $data['eptt_setting_main_product_id'] = $this->config->get('eptt_setting_main_product_id');
        ...
        $data['eptt_setting_price_multiplier_shipping'] = $this->config->get('eptt_setting_price_multiplier_shipping');
        $data['eptt_setting_price_multiplier_desi_shipping'] = $this->config->get('eptt_setting_price_multiplier_desi_shipping');
        $data['eptt_setting_price_multiplier_desi_shipping_status'] = $this->config->get('eptt_setting_price_multiplier_desi_shipping_status');
        $data['eptt_setting_price_multiplier_shipping_status'] = $this->config->get('eptt_setting_price_multiplier_shipping_status');
        $data['eptt_setting_order_status_id'] = $this->config->get('eptt_setting_order_status_id');
        $data['eptt_setting_order_range'] = $this->config->get('eptt_setting_order_range');
        ...
        //BU KISIM HEPSİBURADA İÇİN TAMAMLANACAK..
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $hb_service_key = $this->request->post['hb_service_key'];

            $market = $this->model_entegrasyon_general->getMarketPlace('hb');
            $post_data = array('market' => $market, 'api_info' => array('merchant_id' => $hb_merchant_id,'service_key'=>$hb_service_key), 'request_data' => array());
            ...
            $data['hb_service_key'] = $this->config->get('hb_service_key');
    ...
    public function pz()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $pz_api_anahtari = $this->request->post['pz_api_anahtari'];
            $pz_api_sifresi = $this->request->post['pz_api_sifresi'];
            $pz_satici_numarasi = $this->request->post['pz_satici_numarasi'];
            $params = array(
                'username' => $pz_api_anahtari,
                'password' => $pz_api_sifresi,
                'shopId' => $pz_satici_numarasi
            );
            $this->load->model('entegrasyon/general');

            $market = $this->model_entegrasyon_general->getMarketPlace('pz');
            $post_data = array('market' => $market, 'api_info' => $params, 'request_data' => array());
            $result = $this->entegrasyon->clientConnect($post_data, 'check_api', 'pz',false);
            ...
            if ($result['status']) {
                ...
                $this->load->model('setting/setting');
                $this->model_setting_setting->editSetting('pz', $this->request->post);
                $market['status'] = 1;

                if (!$this->model_setting_setting->getSetting('pz_setting')) {

                    $default_setting = $this->model_entegrasyon_general->pz_default_setting();
                    $this->model_setting_setting->editSetting('pz_setting', $default_setting);
            ...
            $data['token_link'] = $this->token_data['token_link'];
            $data['pz_satici_numarasi'] = $this->config->get('pz_satici_numarasi');
            $data['pz_api_anahtari'] = $this->config->get('pz_api_anahtari');
            $data['pz_api_sifresi'] = $this->config->get('pz_api_sifresi');
            $data['pz_status'] = $this->config->get('pz_status');
            $this->response->setOutput($this->load->view('entegrasyon/setting/api/pz', $data));
        }
    ...
    public function updater()
    {
        ...
        $versiyon_data =false;// $this->model_entegrasyon_general->getVersionInfo($this->model_entegrasyon_updater->version()['version']);
    ...
    public function updatepzdb()
    {
        ...
        $update_sql[] = "ALTER TABLE `" . DB_PREFIX . "es_option` CHANGE `market_option_id` `market_option_id` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL;";
        $update_sql[] = "ALTER TABLE `" . DB_PREFIX . "es_option_value` CHANGE `market_option_value_id` `market_option_value_id` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL";
        $update_sql[] = "ALTER TABLE `" . DB_PREFIX . "es_category` ADD `pz` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `cs`;";
        $update_sql[] = "ALTER TABLE `" . DB_PREFIX . "es_manufacturer` ADD `pz` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `cs`;";
        $update_sql[] = "ALTER TABLE `" . DB_PREFIX . "es_product` ADD `pz` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `cs`;";
        $update_sql[] = "ALTER TABLE `" . DB_PREFIX . "es_product_to_marketplace` ADD `pz` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `cs`;";
        $update_sql[] = "ALTER TABLE `" . DB_PREFIX . "es_option` CHANGE `category_id` `category_id` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL;";
    ...
    public function updatedb()
    {
        ...
        $update_sql = array();
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_market_product` ENGINE = MYISAM;";
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_attribute` ENGINE = MYISAM;";
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_attribute_value` ENGINE = MYISAM;";
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_product_to_marketplace` ENGINE = MYISAM;";
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_product_variant` ENGINE = MYISAM;";
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_product_error` ENGINE = MYISAM;";
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_order` ENGINE = MYISAM;";
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_ordered_product` ENGINE = MYISAM;";
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_option` ENGINE = MYISAM;";
        $update_sql[] = "ALTER TABLE `'".DB_PREFIX."'es_option_value` ENGINE = MYISAM;";
    ...
    public function version()
    {
        ...
        if(isset($json['error'])){
            ...
            // $this->logout();
            $json['status']=false;
            $json['message']="Kayıtlı domain ile şu an kullanmak olduğunuz domain farklı olduğu için çıkış yapıldı, lüfen kullanıcı bilgileriniz ile giriş yapınız.";
            $this->response->addHeader('Content-Type: application/json');
