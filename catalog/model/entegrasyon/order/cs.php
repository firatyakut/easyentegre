...
class ModelEntegrasyonOrderCs extends Model
{
    ...
    public function getFromMarket($order_number=false)
    {
        ...
        if ($is_active) {
            ...
            $baslangicTarihi = new \DateTime(date("Y-m-d H:i:s", strtotime("+1 day")));

            $bitisTarihi = new \DateTime(date("Y-m-d H:i:s", strtotime("-10 day"))); // now()

            $end = $baslangicTarihi->format(\DateTime::RFC3339);
            $start = $bitisTarihi->format(\DateTime::RFC3339);
            ...
            $headers = array(
                "x-api-key: ".$this->config->get('cs_api_anahtari'),
...
//for debug only!
    ...
    public function getOrders($debug=false)
    {
        ...
        $orders = $this->getFromMarket();//$this->entegrasyon->clientConnect($post_data, 'get_orders', 'cs', $debug);


        if (isset($orders['result']['supplierOrderListWithBranch'])) {
    ...
    public function check_order_status($order)
    {
        ...
        $order_detail = $this->getFromMarket();



        $result=array();

        if(isset($order_detail['result']['supplierOrderListWithBranch'])){


           foreach ($order_detail['result']['supplierOrderListWithBranch'] as $csorder) {



$query=$this->db->query("select * from ".DB_PREFIX."es_order where market_order_id='".$csorder['orderId']."'");


               $order_status = isset($csorder['orderProductStatus']) ? $csorder['orderProductStatus'] : '';
               $tracking_url = isset($csorder['shipmentTrackingUrl']) ? $csorder['shipmentTrackingUrl'] : '';
               $cargo_name = isset($csorder['cargoCompany']) ? $csorder['cargoCompany'] : '';
               $cargo_number = isset($csorder['partialNumber']) ? $csorder['partialNumber'] : '';


               $result['cs'][$query->row['order_id']] = array(
                   'status' => $order_status,
...

    {
        ...
        $shipping_address = array(
            ...
            'shipping_zone_id' => $this->entegrasyon->findZoneId(strtolower($sehir)),
    ...
    private function getProductInfo($product)
    {
        ...
        if($product_info['product']){
            ...
            if($this->config->get('cs_setting_model_prefix')){
              ...
              if(count($model2)==2){
                  $product_info = $this->entegrasyon->getProductByOrderModel($product['barcode'],$this->config->get('cs_setting_model_prefix').$model2[1],$product['name'],'cs');
        ...
        $product_data = array(
            ...
            'market_product_id'=>$product['productCode'],
