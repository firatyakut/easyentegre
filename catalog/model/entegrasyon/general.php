...
class ModelEntegrasyonGeneral extends Model {
    ...
    public function read_session($filename)
    {

        $dosyaAdi = DIR_DOWNLOAD . $filename.'.txt';
        $file = fopen($dosyaAdi, 'r');
        $jsonData = fread($file, filesize($dosyaAdi));
        fclose($file);
...
// JSON verisini diziye çevir
    ...
    public function delete_session($filename)
    {

        $this->session_set(array(),$filename);

    }

    public function session_set($array,$filename)
    {

        try {
            $jsonData = json_encode($array);

// Dosyayı aç ve veriyi dosyaya yaz
            $dosyaAdi = DIR_DOWNLOAD . $filename.'.txt';
